...
$(function () {
    ...
    document.addEventListener("scroll", finalpagina);
    function finalpagina() {
        $(window).scroll(function () {
            ...
            let valorPagina= $("#pagina").val();
            valorPagina=parseInt(valorPagina);
            $("#pagina").val(valorPagina+1);
    ...
    function cargarMas() {
      ...
      success: success
         ...
         */
        $.ajax('http://localhost:8000/api/toys/' + $('#pagina').val() + '/' + searchBar,
            {
...
//      let hrefImg = $("<a> </a>").attr("href", "/img/"+data[cont].id+".jpg" ).text();
                   ...
                   let hrefImg = $("<a> </a>").attr({href: "/producto/"+data[cont].id, class: "product-image"}).appendTo(divImg);
                   ...
                   let buttonComprar=$("<button></button>").attr({class: "btn btn-light", type: "button"}).text("Compra Ahora").appendTo(divCol4);
